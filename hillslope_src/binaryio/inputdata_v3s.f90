...
subroutine inputdata2(mode)
        ...
        real    Lch_min,setfsub,setfsurf,qperdx,qtop,cbar
        integer :: ji,ki,dvalint
		...
		if (mode.eq.'restart') then
            ...
            read(150,rec=1) old_q(:,:)
            read(150,rec=2) old_q_ch_in(:,:)
            read(150,rec=3) old_q_ch_out(:,:)
        ...
        do j=1,pfafunits
            ...
            if (mode.eq.'initial') then
                ...
                do k = 1,ndx
                    ...
                    y_pl_s(j,k)=alp_pl_s(j)*(length_p(j)/ndx)*setfsurf*k
                    q_pl_s(j,k)=(y_pl_s(j,k)/alp_pl_s(j))**(1/bet_pl_s(j))
